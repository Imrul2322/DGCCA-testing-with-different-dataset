...
class WeightedGCCA:
  ...
  def __init__(self, V, F, k, eps, truncParam, viewWts=None, verbose=True):
    ...
    self.G_scaled = None # Scaled by SVs (singular values) of covariance matrix sum
  ...
  def _compute(self, views, K=None, incremental=False):
    ...
    # column wise multiplication of weights
    ...
    for i, (eps, view) in enumerate(zip(self.eps, views)):
      # here view means basically outputs of nets
      A, S, B = scipy.linalg.svd(view, full_matrices=False, check_finite=False)
      # working for one view only at a time

      # shape of A is 400 * 2
      # shape of S is 1 * 2
      # shape of B is 2 * 2

      # A is left singular vectors
      ...
      # dimension of T is 2 * 2 diagonal
      # I think T basically is a measure of regularization on covarriance matrix eps is rcov which is Small amount of regularization to apply to covariance matrix for each


      # Keep singular values
      T_unnorm = np.diag( S_thin + eps )

      # I think T and T_unnorm both basically represents singular values with 

      if incremental:
        ajtj = K_invSqrt.dot( np.sqrt(self.W[i]) * A.dot(T) )
        # multiplying A with normalized singular values column wise. A.dot(T)
        # W[i] is a float
        # then multiplying A with K_invSqrt row wise
        ...
        As.append(A[:,:self.truncParam]) # 400 * 2 total 3 items
        Ts.append(T) # 2 * 2 total 3 items
        Ts_unnorm.append(T_unnorm) # 2 * 2 total 3 items
    ...
    if incremental:
      ...
      # columns of G is scaled by lbda_scaled values.
      # G_scaled has same dimension as G
      if self.verbose:
  ...
  def _batch_incremental_pca(x, G, S):
    ...
    # as mode is full, dimension of J is 400 * 400 and dimension of W is 400 * 2
    Q = np.bmat( [[np.diag(S), xh], [np.zeros((b,r), dtype=np.float32), W]] )
    # [[2 * 2, 2 * 2], [400*2, 400 * 2]]
    # dimension of Q is 402 * 4
    ...
    # np.bmat([G, J]) has dimension 400 * 402, G_new[:,:r] has dimension 402 * 2
    return G_new, St_new

...
class TestDGCCA(unittest.TestCase):
  ...
  def testSerializeModel(self):
    ...
    # vIdx = view Id x, lIdx = layer Id x
  ...
  def testDGCCA(self):
    ...
    d = scipy.io.loadmat('../resources/test_with_3Views_3classes.mat')
    self.views = [np.float32(d['view1']), np.float32(d['view2']), np.float32(d['view3'])]
    self.K = np.ones((600,3), dtype=np.float32)
    # self. K = 400 * 3
    ...
    model = DGCCA(arch, lparams, vnames)
    model.build()
    # model.build() basically directs to _model in DGCCAmodel()
    history = []
    self.classes  = ['Class1' for i in range(int(600 / 3))] + ['Class2' for i in range(int(600 / 3))] + ['Class3' for i in range(int(600 / 3))]
    # Plot to PDF
    with PdfPages(os.path.join(self.outDir, 'dgccaData.pdf')) as pdf:
      for epoch in range(2):
        ...
        #  model.learn() line 548
        # things inside the history.extend() will return lowestTuneError
        # Plot output layers
        outputViews = model._model.get_outputs_centered(*self.views)

        # _model will take it to DGCCAmodel
        # [net.output_centered  for net in self.nets]

        for VIdx, output in enumerate(outputViews):
          df = pd.DataFrame(output, columns=['x', 'y'])
          df['Classes'] = self.classes
          # self.classes = ['Class1' for i in range(int(self.N/2))] + ['Class2' for i in range(int(self.N/2))]
          fig = sns.lmplot(x="x", y="y", fit_reg=False, markers=['+', 'o', 'x'], legend=False, hue="Classes", data=df).fig
          plt.legend(loc='best')
          plt.title('View %d Output layer -- Epoch %d' % (VIdx + 1, epoch*200))
          pdf.savefig()
          plt.close(fig)

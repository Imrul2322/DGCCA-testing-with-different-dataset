...
class DGCCAModel:
  ...
  def __init__(self, architecture, rcov=1.e-12, viewWts=1.0, viewNames=None, seed=-1):
    ...
    # for testDGCCA
    # viewMlps = [ [2, 10, 10, 10, 2], [2, 10, 10, 10, 2], [2, 10, 10, 10, 2] ]
    # rcov = [0.01] * 3
    # viewWts=[1.0]*3
    ...
    self.Us = [] # Keep track of weights mapping from network output layer to multi-view layer.
  ...
  def _build(self):
    ...
    for lwidths, vname in zip(self.arch.viewMlps, self.vnames):
      ...
      # there should be 3 individual net
    ...
    self.center_outputs   = theano.function(inputs=self.inputs + K_missing,
                                    ...
                                    updates=[net.Bupdate for net in self.nets])
    ...
    # projections to G
    self.getViewProjs = theano.function(self.inputs, outputs=self.__Gprimes)
    
    # Get average projection or projections of each view
    self.getGmean = theano.function(self.inputs + K_missing, outputs=self.__Gmean)
  ...
  def train(self, lparams, trainViews, trainMissingData, tuneViews=None, tuneMissingData=None, logger=None, calcGMinibatch=False):
    ...
    # trainMissingData has shape 400 * 3
    ...
    # SGD parameters
    bsize = lparams.batchSize # minibatch size, only used in backprop  400
    ...
    minibatch_indices = []
    # math.ceil(trainViews[0].shape[0] / bsize) is 1 as bsize is 400
...
class DGCCA:
  ...
  def learn(self, trainViews, tuneViews=None, trainMissingData=None, tuneMissingData=None, embeddingPath=None, modelPath=None, logPath=None, calcGMinibatch=False):
    ...
    for epoch, trainErr, tuneErr, epochTime in self._model.train(self.lparams, trainViews, trainMissingData, tuneViews, tuneMissingData, logger=None, calcGMinibatch=calcGMinibatch):
      # self._model.train gives yield epoch, trainErr, tuneErr, epochTime
